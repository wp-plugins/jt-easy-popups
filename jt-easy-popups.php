...
function JTPOP_enqueue_scripts(){
    ...
    wp_enqueue_style('JTOPO-admin-style', Plugins_url('/css/jt-popup-admin.css', __FILE__), false, '1.0.0');
}
...
function JTPOP_create_menu(){
    add_menu_page('STUDIO-jt POPUP', 'JT Easy Popups', 'administrator', __FILE__, 
                'JTPOP_settings_page', Plugins_url('/images/jt-icon.png', __FILE__));
...
<script type="text/javascript">
    jQuery(document).ready(function($) {
        ...
        $("section .jt_save_popup").click(function(){
            ...
            if(!$(".jtpop_select", inputsection)){
                saveval[8] = "jtpop_white";
            }else{
                saveval[8] = $(".jtpop_select", inputsection).attr("id");
            }
...
function JTPOP_popup_list(){
    ...
    if ($wpdb->get_var("SELECT * FROM `" . $table . "`") != "") {
        ...
        <section class="jtpop_section">
            <div class="section_nav"><p class="pc_version_ck">POPUP <?php echo $value -> rowid ?></p><p class="delete_popup"><img src="<?php echo Plugins_url('/images/close.png', __FILE__); ?>" alt="popup delete"/><input type="hidden" class="jtpop_row" value="<?php echo $value -> rowid; ?>" /></p><p class="small_section"><img class="smallsize" src="<?php echo Plugins_url('/images/small.png', __FILE__); ?>" alt="sections size change"></p></div>
            <article class="pc_option">
                <ul class="popup_img">
                    <li class="jt-cuztom-td">
                            ...
                            echo "<span class=\"cuztom-preview\"><img src=\"$image_attributes[0]\" width=\"180\" height=\"127\" alt=\"popup image\" /></span></br >"; 
                ...
                <ul class="option_list">
                    ...
                    <li>팝업의 상단 여백을 <input type="text" class="jtpop_top" value="<?php echo $value -> popuptop; ?>" />px, 좌측 여백을<input type="text" class="jtpop_left" value="<?php echo $value -> popupleft; ?>" />px로 합니다.<br>
                       [동일한 기간에 팝업이 하나일때, 상단과 좌측에 여백을 입력하지 않으면 중앙에 위치합니다.]
                    </li>
                    <li class="form_title">팝업의 개시 기간을 <input type="text" class="pop_date jtpop_startdate" value="<?php echo $value -> startdate; ?>" />부터 <input type="text" class="pop_date jtpop_enddate" value="<?php echo $value -> enddate; ?>" />까지로 합니다.</li>
                    <li>두개 이상의 팝업이 겹칠때 이 팝업의 우선순위를 <input type="text" class="jtpop_zindex" value="<?php echo $value -> zindex; ?>" maxlength="1" />로 합니다.<br />
                       [ 0 ~ 9, 숫자가 클수록 우선순위 높음.]
                    </li>
                    <li>팝업의 '닫기' 버튼을 <span id="jtpop_black" class="close_color" <?php 
                    if($value->closecolor == "jtpop_black"){
                        echo 'style="color:#f7938e; font-weight:bold;"';
                    }
                    if($value->closecolor == "jtpop_white"){
                        echo 'style="font-weight:normal; color:#666;"';
                    }?>
                    >검은색</span> 또는 <span id="jtpop_white" class="close_color" <?php 
                    if($value->closecolor == "jtpop_white"){
                        echo 'style="color:#f7938e; font-weight:bold;"';
                    }
                    if($value->closecolor == "jtpop_black"){
            ...
            <div class="jt_save_box">
                <div class="jt_save_popup">
                    <img src="<?php echo Plugins_url('/images/save_icon.png', __FILE__); ?>" alt="Add Icon" /> &nbsp;저장 
                </div>
                <div class="loading_icon"><img src="<?php echo Plugins_url('/images/loading.gif', __FILE__); ?>" alt="loading" /></div>
...
<div id="jt_pop_admin_page">
    <div id="jtpop_header">
        <img src="<?php echo Plugins_url('/images/jt_logo.png', __FILE__); ?>" alt="STUDIO JT" />
    ...
    <div id="popups_option_container">
        ...
        <?php settings_fields('JTPOP_bs_setting'); ?>
            ...
            <div class="save_box">
                ...
                <div class="loading_icon2 basic_save_loading"><img src="<?php echo Plugins_url('/images/loading.gif', __FILE__); ?>" alt="loading" /></div>
                <div id="jt_reset">초기화</div>
                <div class="loading_icon2 reset_loading"><img src="<?php echo Plugins_url('/images/loading.gif', __FILE__); ?>" alt="loading" /></div>
                <div style="clear:left;"></div>
            </div>
        </form>
    </div>
    ...
    <div id="jtpop_menu">
        <p id="jt_add_popup">
            <img src="<?php echo Plugins_url('/images/add_icon.gif', __FILE__); ?>" alt="Add Icon" />
         &nbsp;팝업 추가</p>
        <p id="add_loading"><img src="<?php echo Plugins_url('/images/loading.gif', __FILE__); ?>" alt="loading" /></p>
    ...
    <div id="jtpop_footer">
        <div id="footer_help">
            <ul>
                <li>팝업은 테마의 첫 페이지에만 나타납니다.</li>
                <li>팝업은 최대 8개까지 등록할 수 있습니다.</li>
            </ul>
        </div>
    </div>
...
function JTPOP_show_html(){
    ...
    foreach ($fivesdrafts as $key => $value) {
        ...
        if($pcpopupsize == 1 && $value->popuptop == "" && $value->popupleft == ""){
            echo '<article class="jtpop-bgs">';
        }else{
            echo '<article class="jtpop-bgs" style="top:' .$value->popuptop. 'px; left:'.$value->popupleft. 'px; z-index:99999'.$value->zindex.'">';
        }
        ?>
            <div class="jtpop_bg_page">
                <div class="popclose"><img src="
                <?php if( $value->closecolor == "jtpop_black"){
                        echo Plugins_url('/images/close_pop.png', __FILE__); 
                      }else{
                        echo Plugins_url('/images/close_pop_white.png', __FILE__);
                      }?>" alt="close" ?></div>
                <div class="jt-popup">
                    <div>
                        <?php if($value -> imglink != "" && $value -> imglink != "http://"){
                                  echo '<a href="' . $value -> imglink . '"><img src="'. $image_attributes[0] . '" alt="notice popup img"/></a>';
                              }else{
                                  echo '<img src="'. $image_attributes[0] . '" alt="notice popup img"/>';
                        }?>
                    </div>
                </div>
                <div class="popip_footer">
                    <span class="notoday"><img src="
                        <?php if( $value->closecolor == "jtpop_black"){
                                  echo Plugins_url('/images/close_pop2.png', __FILE__); 
                              }else{
                                  echo Plugins_url('/images/close_pop2_white.png', __FILE__); 
